...
//只能输入0-200并可以保留一位小数
  const checkAge2 = (rule, value, callback) => {
	if(value){
	  if (!/^(?:[1-9][0-9]{0,3}$|[1-9][0-9]{0,3}\.[0-9]|0(?:\.[1-9])?)$/.test(value)) {
	  callback(new Error('请输入数字值'))
	  } else {
	  if (value > 200) {
		callback(new Error('不能超过200'))
...
module.exports = {
	rules:{
		...
		requiredEnginePower:[{ required: true, message: "发电所需油机功率", trigger: "change" }],
		...
		enduranceTime:[{ required: true,validator: checkAge2, trigger: "blur" }],
		// regStatus:[{ required: true, message: "注册状态", trigger: "change" }],状态未确定暂时为非必填
		ratedPower:[{ required: true,validator: checkAge, trigger: "blur" }],
		ratedWorkingVoltage:[{ required: true, message: "额定工作电压（V）", trigger: "blur" }],
		tankVolume:[{ required: true,validator: checkAge, trigger: "blur" }],
	}
};
