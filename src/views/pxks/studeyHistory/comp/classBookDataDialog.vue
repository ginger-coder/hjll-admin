<template>
	<div>
		...
		>
			<div style="height: 600px; padding-bottom: 50px">
				<mTable
					...
					stripe
					:columns="columns"
					:request="learnStudyRecordCourseBookData"
					:payload="queryParams"
					:alertText="true"
				>
					...
					>
						<template slot-scope="scope">
							<el-button
								type="text"
								@click="checkInfo(scope.row.courseContentId)"
								>详细数据</el-button
							>
						</template>
					</el-table-column>
		...
		<el-dialog title="人员完成详情" :visible.sync="infoDrawer" width="800px">
			<mTable
				ref="infoTable"
				:loading.sync="infoLoading"
				stripe
				:columns="infoColumns"
				:request="learnStudyRecordCourseDeatilData"
...
import { learnStudyRecordCourseDeatilData, learnStudyRecordCourseBookData } from '@/api/pxks';

export default {
	name: 'classBookDataDialog',
	components: {
		mTable,
	},
	watch: {
		visible(value) {
			if (value) {
				setTimeout((el) => {
                    ...
                    this.infoQueryParams.courseId = this.courseId;
	...
	methods: {
		learnStudyRecordCourseDeatilData,
		learnStudyRecordCourseBookData,
		checkInfo(id) {
			this.infoQueryParams.courseContentId = id;
