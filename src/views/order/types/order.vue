		...
		<el-scrollbar wrap-class="app-container-wrapper">
			<div ref="pageHead">
				...
				<div v-if="perOperate" class="page_handle_group">
					<el-button type="primary" icon="el-icon-plus" v-hold-click @click="jump('/types/create')">新增</el-button>
				</div>
				<!-- 已选 -->
			...
			<template>
				<el-table :data="lists" ref="lists" fit highlight-current-row border class="no_border_table" v-loading.body="loading" element-loading-spinner="el-icon-loading" element-loading-custom-class="custom-loading" :max-height="tableMaxHeight" :row-key="getRowKey" @selection-change="handleSelectionChange" @cell-click="cell_click">
					...
					<el-table-column label="类型名称" min-width="100" prop="name" fixed show-overflow-tooltip>
						<template slot-scope="scope">
							{{ scope.row.name || "-" }}
						</template>
					</el-table-column>
							<el-table-column label="服务类型" min-width="100" prop="parentCodeName" fixed show-overflow-tooltip>
						<template slot-scope="scope">
							{{ scope.row.parentCodeName || "-" }}
						</template>
					</el-table-column>
...
export default {
	...
	methods: {
		...
		doCommand(command, row) {
			switch (command) {
				case "info":
					this.jump(`/types/info?id=${row.id}`);
					break;
				case "edit":
					this.jump(`/types/create?id=${row.id}`);
					break;
				case "delete":
		...
		openDialog:function(row){
			var that = this
			this.$refs.orderCreate.openDialog(row)
		},
	},
};
</script>
