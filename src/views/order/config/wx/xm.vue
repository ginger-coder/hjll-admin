<template>
  <div class="app-container disp_flex flex_column">
          ...
          <div ref="pageHead">
            <el-form ref="payload" :model="payload" class="form_horizontal">
              <el-form-item label="维修项目名称：" prop="name">
                <el-input placeholder="请输入维修项目名称" clearable v-model="payload.name" @clear="do_search"
                          @keyup.enter.native="do_search"></el-input>
              </el-form-item>
              <el-form-item label="维修类型：" prop="orderDetailsType">
      ...
      <el-card shadow="hover" class="flex mt-space table-card" :body-style="{height: '100%', paddingBottom: 0}">
        <template>
          <m-table height="100%" :columns="columns"  :request="getPollingItemPage" :payload="payload" :loading.sync="loading" ref="table" >
            ...
            <template v-slot="{ row }">
              <!-- <el-button type="text" v-hold-click @click.stop="doCommand('info', row)" style="margin-right: 5px;" v-if="row.status==0"> 配置维修要求</el-button>
              <el-button   type="text" disabled style="margin-right: 5px;" v-if="row.status==1"> 配置维修要求</el-button> 
              ...
              <el-dropdown @command="doCommand($event, row)">
                ...
                <el-dropdown-menu slot="dropdown">
                  <el-dropdown-item command="info" style="margin-right: 5px;" v-if="row.status==0">配置维修要求</el-dropdown-item>
    ...
    <addPatrolWxConfig :saveJump.sync="saveJump" ref="addPatrolWxConfig" @success="dialogCancelAfter"></addPatrolWxConfig>
...
import {xmConfig,getPollingItemPage,getPollingType} from "@/api/orderWxConfig";
import {getLabel, getClass, orderStatus} from "@/utils/dict";
import {deepClone} from "@/utils";
import ConfigFormDialog from '../../components/wxConfigFormDialog.vue';
import addPatrolWxConfig from '../../components/addPatrolWxConfig.vue'
...
export default {
  components: {ConfigFormDialog,mTable,addPatrolWxConfig,settingAmount},
  data() {
    return {
      temStatusLists: [
        {label: '开启', value: 0},
      ...
      columns: [
				...
				{label: '维修项目名称', prop: 'name'},
				{label: '维修类型', prop: 'orderSecondTypeName'},
  ...
  methods: {
    ...
    // 维修类型
    getPollingTypeList(){
      getPollingType({},{orderTypeCode:'WX-TYPE-369'}).then(res=>{
    ...
    table_cell_config(info){
      this.$refs.addPatrolWxConfig.openDialog(info)
    },
