      ...
      <el-card shadow="hover" :body-style="{padding: 10}">
        <div ref="pageHead">
          <el-form ref="searchs" :model="searchs" class="form_horizontal">
            <el-form-item label="维修模块名称：" prop="name">
            ...
            <el-form-item label="维修类型：" prop="orderSecondType">
      ...
      <el-card shadow="hover" class="flex mt-space table-card" :body-style="{height: '100%', paddingBottom: 0}" style="margin-top: 24px;">
        <template>
          ...
          <el-table-column  label="操作" align="center" width="180" slot="Action" fixed="right">
              <template v-slot="{ row }">
                <el-button type="text" v-hold-click @click.stop="doCommand('info', row)"> 配置维修项目</el-button>
...
import {mkConfig,getTableData,temStatusLists,getPollingType,updatePollingModule,removePollingModule} from "@/api/orderWxConfig";
import mTable from '@c/business/table'
import {getLabel, getClass, orderStatus} from "@/utils/dict";
import {deepClone} from "@/utils";
import ConfigFormDialog from '../../components/addModulesWx.vue';
...
export default {
  ...
  methods: {
    ...
    doCommand(command, row) {
      switch (command) {
        case "info":
          this.jump(`/wx/mkSet?id=${row.id}&orderDetailsType=${row.orderDetailsType}&&defaultCurrent=${this.defaultCurrent}&&defaultSize=${this.defaultSize}&&searchs=${JSON.stringify(this.searchs)}&row=${JSON.stringify(row)}`);
          break;
        case "edit":
