<template>
  <div class="app-container">
    <el-scrollbar wrap-class="app-container-wrapper">
      <div ref="pageHead">
        <el-form ref="searchs" :model="searchs" class="form_horizontal">
          <el-form-item label="模块名称：" prop="name">
            <el-input placeholder="请输入模块名称" clearable v-model="searchs.name" @clear="do_search"
                      @keyup.enter.native="do_search"></el-input>
          </el-form-item>
          <el-form-item label="开启状态：" prop="status">
            <el-select v-model="searchs.status" clearable @change="do_search">
              <el-option v-for="item in temStatusLists" :key="'workOrderStatus' + item.value" :label="item.label"
      ...
      <template>
        <el-table :data="lists" ref="lists" fit highlight-current-row border class="no_border_table"
          ...
          <el-table-column label="巡检类型" prop="orderTypeDetailsIdName" min-width="140">
            <template slot-scope="scope">
          ...
          <!--						</template>-->
          <!--					</el-table-column>-->
          <el-table-column label="巡检模块数量统计" prop="moduleCount">
            <template slot-scope="scope">
              {{ scope.row.moduleCount || 0 }}
            </template>
          </el-table-column>
          <el-table-column label="状态" width="120" align="center" prop="status">
            <template slot-scope="scope">
              ...
              <el-switch v-model="scope.row.status" active-color="#13ce66" inactive-color="#909399" :active-value='0'
          ...
          <el-table-column v-if="perOperate" label="操作" align="center" width="160" fixed="right">
            <template v-slot="{ row }">
              <el-button type="text" v-hold-click @click.stop="doCommand('info', row)"> 配置巡检模块</el-button>
    ...
    <config-form-dialog ref="configFormDialog" :pageFun="pageFun" :moduleType="'Template'" :title="'模块'"
                        @success="do_search"></config-form-dialog>
  </div>
</template>
<script>
import basic from "@/mixins/basic";
import {mbConfig} from "@/api/orderXjConfig";
...
export default {
  ...
  methods: {
    ...
    doCommand(command, row) {
      switch (command) {
        case "info":
          this.jump(`/xj/mbSet?id=${row.id}&typeDetailsIds=${row.orderTypeDetailsId}`);
          break;
        case "edit":
    ...
    table_cell_state_change(row, status) {
      let that = this;
      row.status = status
      that.pageFun({name: 'updateTemplate', method: 'post'}, {}, row)
          .then(res => {
