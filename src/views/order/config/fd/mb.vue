<template>
	<div class="app-container">
		<el-scrollbar wrap-class="app-container-wrapper">
			...
			<template>
				<el-table :data="lists" ref="lists" fit highlight-current-row border class="no_border_table" v-loading.body="loading" element-loading-spinner="el-icon-loading" element-loading-custom-class="custom-loading" :max-height="tableMaxHeight" :row-key="getRowKey" @selection-change="handleSelectionChange" @cell-click="cell_click">
					...
					<el-table-column label="适用地区" min-width="100" prop="name" fixed show-overflow-tooltip>
						<template slot-scope="scope">
							{{ scope.row.provinceName+'-'+ scope.row.cityName|| "-" }}
						</template>
					</el-table-column>
					<el-table-column label="基础时长（小时）" min-width="100" prop="name" fixed show-overflow-tooltip>
						<template slot-scope="scope">
							{{ scope.row.duration || "-" }}
						</template>
					</el-table-column>
					<el-table-column label="油价计费方式" min-width="100" prop="name" fixed show-overflow-tooltip>
						<template slot-scope="scope">
							{{ scope.row.oilPricingTypeName || "-" }}
						</template>
					</el-table-column>
					<el-table-column label="油价（元）固定价格时展示" min-width="100" prop="name" fixed show-overflow-tooltip>
						<template slot-scope="scope">
							{{ scope.row.oilPricingType || "-" }}
						</template>
					</el-table-column>
					...
					<el-table-column v-if="perOperate" label="操作" align="center" width="160" fixed="right">
						<template v-slot="{ row }">
							<el-button type="text" v-hold-click @click.stop="doCommand('info', row)"> 配置</el-button>
		...
		<config-form-dialog ref="configFormDialog" :pageFun="pageFun" :moduleType="''" :title="'模板'" @success="do_search" ></config-form-dialog>
	</div>
</template>
<script>
import basic from "@/mixins/basic";
import { mbConfig } from "@/api/orderFdConfig";
