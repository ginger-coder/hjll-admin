					...
					<div v-for="(item,index) in form.requireListInfo" :key="item.id" >
						<div class="card_item disp_flex  ali_cen ju_bet">
							...
							<div class="card_item_box disp_flex ali_cen">
								<el-input style="margin-right:10px;width:200px" v-model="item.requireInfo" placeholder="请输入巡检要求"></el-input>
...
import { hjConfig, yqConfig, requireConfig } from "@/api/orderFdConfig";
...
export default {
	...
	methods: {
		...
		save:function(){
			...
			requireConfig({method:'post',name:'dynamoListBatch'},{},{linkId:that.form.id,list:_postJson}).then((res)=>{
				var post = {
