    ...
    <el-dialog title="配置巡检要求"
      ...
      <!-- <el-scrollbar ref="dialogScrollbar" wrap-class="dialog-wrapper"> -->
        <el-form class="el-form-dialog" ref="form" :model="form" :rules="rules">
          <el-form-item label="巡检项目类型：" prop="requireType">
            <el-select filterable clearable v-model="form.requireType" >
          ...
          <el-form-item  label="巡检要求：" prop="requireInfo">
            <el-input v-model="form.requireInfo" type="textarea" :rows="4" placeholder="'巡检要求内容填写"></el-input>
          </el-form-item>
...
export default {
  ...
  methods: {
    ...
    getItemByCodeList: function () {
      var that = this
      getItemByCode({groupCode:100}, {}).then(res => {
    ...
    getRequireData(requireList){
      getRequire({name:'getRequire'},{requireId:requireList},{
        ...
        if(res.code==0){
          ...
          console.log(this.allCheckItemsLists)
        }
      })
    },
    openDialogAfter() {
