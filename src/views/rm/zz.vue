<template>
    ...
    >
	  ...
	  <div class="table-box">
		<searchForm  :type="'基站'" @onSearch="onSearch" @onReset="onReset" @onAdd="onAdd" :btnItems="'search,reset,export'"/>
  ...
  export default {
	...
	methods: {
	  ...
	  submitPopupData () {
		// 请求删除接口
		deleteStation(this.row.id).then((res)=>{
			if(res.code == 0){
				this.dialogVisible = false
	  ...
	  // 获取数据
	  getData() {
		getStationData(this.pageObj.pageData).then((res)=>{
			if(res.code == 0){
