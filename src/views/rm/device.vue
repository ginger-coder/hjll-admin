<template>
  <div class="device-contioner">
    ...
    <div class="table-box">
      <searchForm  :type="'设备'" @onSearch="onSearch" @onReset="onReset" @onAdd="onAdd" :btnItems="'search,reset,export'"/>
...
export default {
  ...
  methods: {
    ...
    submitPopupData () {
      // 请求删除接口
      deleteDevice(this.row.id).then((res)=>{
        if(res.code == 0){
          this.dialogVisible = false
    ...
    // 获取数据
    getData() {
      getDeviceData(this.pageObj.pageData).then((res)=>{
          if(res.code == 0){
