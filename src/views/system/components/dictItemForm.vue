    ...
    <div>
        <el-dialog :title="(type!='create' ? '编辑' : '添加') + title" :visible.sync="isShow" fullscreen :lock-scroll="true" :before-close="dialogCancel">
            <el-scrollbar ref="dialogScrollbar" wrap-class="dialog-wrapper">
                <el-form class="userForm" ref="form" :model="form" :rules="rules" label-width="120px">
...
import {dictItemUpdate, dictItemCreate } from "@/api/dict"
            ...
            that.$refs["form"].validate((valid) => {
                if (valid) {
                    ...
                    let next = (post) => {
                        ...
                        let fun = that.type =='modify'? dictItemUpdate :dictItemCreate
                        fun({}, param, post)
                            .then(() => {
                                that.$message({
                                    message: '提交成功',
