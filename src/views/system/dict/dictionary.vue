<template>
    <div class="app-container ">
		<el-scrollbar wrap-class="app-container-wrapper">
			<div ref="pageHead">
                <el-form ref="searchs" :model="searchs" class="form_horizontal">
                    <el-form-item label="字典名称：" >
                        <el-input  placeholder="请输入字典名称" clearable v-model="searchs.keywords" @clear="do_search" @keyup.enter.native="do_search">
                        </el-input>
                    </el-form-item>
                    <el-form-item>
			...
			<template>
				<el-table :data="lists" ref="lists" :border="false" fit highlight-current-row :max-height="tableMaxHeight" :row-key="getRowKey" @selection-change="handleSelectionChange" @cell-click="cell_click">
					...
					<el-table-column label="字典名称" min-width="150" prop="name" >
						<template slot-scope="scope">
							{{ scope.row.name || "-" }}
						</template>
					</el-table-column>
					<el-table-column label="字典编码" min-width="150" prop="name" >
						<template slot-scope="scope">
							{{ scope.row.code || "-" }}
						</template>
					</el-table-column> 
					...
					<el-table-column label="操作" align="center" width="160" fixed="right">
						<template slot-scope="scope">
							...
							<el-button type="text" v-hold-click @click.stop="table_jump(scope.row)">字典项</el-button>
						</template>
					</el-table-column>
				</el-table>
			</template>
        ...
        <dictForm :title="moduleTitle" ref="formDialog" @submit="formDialogSubmit" />
    </div>
</template>
<script>
import basic from "@/mixins/basic"

import { dictPages,dictRemove,dictUpdate,dictInfo } from "@/api/dict"
...
export default {
    ...
    methods: {
		...
		table_cell_state_change(row, status) {
			let that = this;
			dictUpdate({},{},{...row,status:row.status})
				.then(res => {
        ...
        table_jump:function(row){
            ...
            // this.jump(`/system/dictData?typeName=${row.name}&typeCode=${row.code}`);
            this.jump(`/system/type/data?typeName=${row.name}&typeCode=${row.code}`);
            // this.jump(`/system/data?typeName=${row.name}&typeCode=${row.code}`);
        },
    },
};
</script>
