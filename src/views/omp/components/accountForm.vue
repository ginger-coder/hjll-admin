<template>
  <div>
    <el-dialog :title="(type!=='create' ? '编辑' : '添加') + title" :visible.sync="isShow" fullscreen :lock-scroll="true"
      ...
      <el-scrollbar ref="dialogScrollbar" wrap-class="dialog-wrapper">
        <el-form class="el-form-dialog" ref="form" :model="form" :rules="rules">
          ...
          <el-form-item v-if="org" class="el-form-item-half" label="部门：" prop="rootIds">
            <el-cascader ref="orgCascader" v-model="form.rootIds" :options="orgData"
                         :props="{ checkStrictly: true,expandTrigger: 'hover' }" :show-all-levels="false"></el-cascader>         </el-form-item>
          ...
          <el-form-item class="el-form-item-half" label="角色：" prop="roleno">
            <el-select v-model="form.roleno" clearable>
              <el-option v-for="item in roleData" :key="'role'+item.value" :label="item.label"
