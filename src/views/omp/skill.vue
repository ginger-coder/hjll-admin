      ...
      <div ref="pageHead">
        <el-form ref="searchs" :model="searchs" class="form_horizontal">
          <el-form-item label="技能：" prop="name">
            <el-input placeholder="请输入技能" clearable v-model="searchs.name" @clear="do_search"
        ...
        <el-button type="primary"  icon="el-icon-plus" class="add" @click="jump('/omp/createskill')">新建</el-button>
      ...
      <template>
        <el-table :data="lists" ref="lists" fit highlight-current-row border class="no_border_table"
          ...
          <el-table-column label="运维人员数量" width="200" prop="peopleNum">
            <template v-slot="{row}">
              <span  @click.stop="table_cell_info(row)">{{ row.peopleNum || "-" }}</span>
            </template>
          </el-table-column>
          <el-table-column label="技能编码" width="200" prop="code">
            <template v-slot="{row}">
              <span>{{ row.code ||"-" }}</span>
            </template>
          </el-table-column>
          <el-table-column label="状态" prop="status">
            <template v-slot="{row}">
              <div class="status">
                <span :class="[`${row.status}` === `1` ? 'dot-open' : 'dot-close']">●</span>
...
import {page, update } from "@/api/workSkill";
import {delskill } from "@/api/createskill";
export default {
  components: {
    accountForm,
    accountInfo,
    ...
    async changeStatus(id, val) {
     ...
     doCommand(command, row) {
      switch (command) {
        ...
        this.$router.push({
        path: '/omp/createskill',
        query:{allData: encodeURIComponent(objData)}
    })
					break;
          case "info":
           let objData2 = JSON.stringify(row)
        this.$router.push({
        path: '/omp/createskill',
        query:{allData: encodeURIComponent(objData2),flag:true}
    })
					break;
          case "del":

          delskill({id:row.id}).then(res=>{
            console.log(res);
