      ...
      <template>
        <el-table :data="lists" ref="lists" fit highlight-current-row border class="no_border_table"
          ...
          <el-table-column label="验证结果" prop="checkStatus" :filters="stateData"
            ...
            <template v-slot="{row}">
              ...
              >
                ...
                <template #reference>
                  <span @click.stop="()=>showVerify(row)" class="btn-verify">验证</span>
                </template>
              </el-popover>
            </template>
          </el-table-column>
          ...
          <el-table-column v-if="perOperate" label="操作" align="center" width="138" fixed="right">
            <template v-slot="{row}">
              <el-button type="text" v-hold-click style=""
                         @click.stop="doCommand('audit',row)">
                审核
              </el-button>
...
export default {
  ...
  data() {
    return {
      pageFun: (opt, data, post) => checkList(opt, {authenStatus: 0, ...data}, post),
      moduleTitle: "账号",
      apiKey: "adminId",
  ...
  methods: {
    ...
    doCommand(command, row) {
      switch (command) {
        case "info":
          this.jump(`/isp/ispAuditInfo?id=${row.id}`)
          break;
        case "audit":
