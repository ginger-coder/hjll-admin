<template>
  <div class="tabs-wrapper">
    <div class="tool">
      <span v-for="(t,i) in date.label" :key="i" :class="[date.select === t ? 'selected':'']" @click="()=>clickDate(t)">{{
          t
        }}</span>
      <el-date-picker
          v-model="date.value"
    ...
    <el-tabs v-model="activeName" class="tabs" @tab-click="changeTab">
      <el-tab-pane label="工单量" name="1" :lazy="true">
        <div class="pane">
          <div class="l">
...
export default {
  ...
  methods: {
    async fetchData(timeType) {
      const res = await request({
        url: "system/home/statisticalData?timeType=" + timeType,
        // url: "user/home/moneyAndOrder?sign=" + timeType,
        method: "get"
      });
      if (res && res.code == 0) {
        // const {dates, money, order, companyInfoList, companyInfoS, dealMoneyTypeRatioList} = res.data;
        const {trendChartList} = res.data
        // mock的数据
        // var trendChartList = [
        //   {
        //     abscissa:["2023-06-05","2023-06-06","2023-06-07","2023-06-08","2023-06-09","2023-06-10","2023-06-11"],
        //     ordinate:['3','4','45','87','87','23','5','7','56'],
        //     type:'1'
        //   },
        //   {
        //     abscissa:["2023-06-05","2023-06-06","2023-06-07","2023-06-08","2023-06-09","2023-06-10","2023-06-11"],
        //     ordinate:['10','12','35','65','76','78','45','97','56'],
        //     type:'2'
        //   }
        // ]
        // let orderPieList = [
        ...
        //   }
        // ]
        // res.data.pieChartsList[0]?.orderPieList
        let pieData = res.data.pieChartsList[0]?.orderPieList
        trendChartList.forEach(element => {
          if(element.type =='1'){
            this.money = element.ordinateCount
          }
          if(element.type =='2'){
            this.order = element.ordinateCount
          }
        });
        ...
        this.dates = trendChartList[0].abscissa
        // const defaults = ['巡检', '发电', '故障'].map(n => {
        //   return {workOrderTypeName: n, orders: 0, money: 0}
        // })
        this.pieData = (pieData.length === 0 ? defaults : pieData).map((item, i) => {
          return {
            ...item,
            name: item.type,
            money: item.count,
    ...
    changeTab(e) {
      this.$nextTick(() => {
        ...
        if (this.bar) {
          ...
          barOp.series[0].data = this.order
    ...
    async changeDate(arr) {
      ...
      const res = await request({
        url: `system/home/statisticalData?startTime=${s}&endTime=${e}&timeType=scope`,
        // url: `user/home/moneyAndOrder?startTime=${s}&endTime=${e}`,
        method: "get"
      });
      if (res && res.code == 0) {
        // const {dates, money, order, companyInfoList, companyInfoS, dealMoneyTypeRatioList} = res.data;
        ...
        this.dates = trendChartList[0].abscissa
        this.pieData = pieData.map((item, i) => {
          return {
            ...item,
            name: item.type,
            money: item.count,
